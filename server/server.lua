...
Citizen.CreateThread(function()
    ...
    PerformHttpRequest('https://raw.githubusercontent.com/kac5a/pedy_dokumentum/master/fxmanifest.lua',function(error, result, headers)
...
RegisterCallback('pedy_dokumentum:getPlayerCopies', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)

  MySQL.Async.fetchAll(
    'SELECT id, data, isCopy FROM pedy_dokumentum WHERE ownerId = @identifier and isCopy = "1"', {
...
if Config.DocumentItemName then
  RegisterItem(Config.DocumentItemName, function(source)
    local src = source
    TriggerClientEvent("pedy_dokumentum:useItem", src)
  end)
end

RegisterCallback('pedy_dokumentum:getPlayerDocuments', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)

    MySQL.Async.fetchAll(
      'SELECT id, data, isCopy FROM pedy_dokumentum WHERE ownerId = @identifier and isCopy = "0"', {
      ['@identifier'] = PlayerIdentifier
    }, function(result)
      local mappedResult = {}
      for k, v in pairs(result) do
...
RegisterCallback('pedy_dokumentum:getDocumentTemplates', function(source, cb)
  local src = source  
  local PlayerJobName = GetPlayerJobName(src)

  MySQL.Async.fetchAll(
    'SELECT id, data FROM pedy_dokumentum_templates WHERE job = @job', {
    ['@job'] = PlayerJobName
  }, function(result)
    local mappedResult = {}
    for k, v in pairs(result) do
...
RegisterCallback('pedy_dokumentum:createTemplate', function(source, cb, data)
  local src = source  
  local PlayerJobName = GetPlayerJobName(src)

  MySQL.Async.insert('INSERT INTO pedy_dokumentum_templates (data, job) VALUES (@data, @job)', {
			['@data'] = data,
			['@job'] = PlayerJobName
		}, function(result)
      cb(result)
...
RegisterCallback('pedy_dokumentum:editTemplate', function(source, cb, data)
  local obj = json.decode(data)

  MySQL.Async.execute('UPDATE pedy_dokumentum_templates SET data = @data WHERE id = @id', {
			['@data'] = data,
			['@id'] = obj.id
		}, function(result)
      cb(result)
...
RegisterCallback('pedy_dokumentum:deleteTemplate', function(source, cb, data)
  MySQL.Async.execute('DELETE FROM pedy_dokumentum_templates WHERE id = @id', {
			['@id'] = data
		}, function(result)
      cb(result)
  end)
...
RegisterCallback('pedy_dokumentum:getPlayerData', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)
...
RegisterCallback('pedy_dokumentum:createDocument', function(source, cb, data)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)

  MySQL.Async.insert('INSERT INTO pedy_dokumentum (data, ownerId, isCopy) VALUES (@data, @ownerId, @isCopy)', {
			['@data'] = data,
...
RegisterServerEvent('pedy_dokumentum:createServerDocument')
AddEventHandler('pedy_dokumentum:createServerDocument', function(data)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)
...
RegisterCallback('pedy_dokumentum:deleteDocument', function(source, cb, data)
  MySQL.Async.execute('DELETE FROM pedy_dokumentum WHERE id = @id', {
			['@id'] = data
		}, function(result)
      cb(result)
  end)
...
RegisterServerEvent('pedy_dokumentum:giveCopy')
AddEventHandler('pedy_dokumentum:giveCopy', function(data, targetId)
  local src = source
  local tsrc = targetId
  ...
  MySQL.Async.insert('INSERT INTO pedy_dokumentum (data, ownerId, isCopy) VALUES (@data, @ownerId, @isCopy)', {
    ...
    TriggerClientEvent("pedy_dokumentum:copyGave", src, data.name)
    TriggerClientEvent("pedy_dokumentum:copyReceived", tsrc, data.name)
  end)

end)

RegisterServerEvent("pedy_dokumentum:receiveDocument")
AddEventHandler("pedy_dokumentum:receiveDocument", function(data, targetId)
  local tsrc = targetId

  MySQL.Async.fetchAll('SELECT data FROM pedy_dokumentum WHERE id = @docId', {
    ['@docId'] = data.docId,
  }, function(result)
    TriggerClientEvent("pedy_dokumentum:viewDocument", tsrc, result[1])
  end)
end)
